...
from sympy import sympify, diff, latex, symbols, integrate, limit, oo
    ...
    with col1:
        ...
        if st.button("Differentiate"):
            try:
                ...
                st.latex(f"\\frac{{d}}{{dx}}({latex(sympify(expr_str))}) = {latex(derivative)}")
        ...
        if st.button("Calculate Limit"):
            try:
                ...
                st.latex(f"\\lim_{{x \\to {latex(sympify(a_str))}}}}} {latex(sympify(expr_str))} = {latex(lim)}")
            except Exception as e:
                st.error(f"Error: {e}")
