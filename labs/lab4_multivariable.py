...
def calculate_multivariable_data(f_str, x_min, x_max, y_min, y_max, x0, y0):
    x, y = symbols('x y')
    f = sympify(f_str)
...
def run_lab4():
    ...
    try:
        data = calculate_multivariable_data(f_str, x_min, x_max, y_min, y_max, x0, y0)
        ...
        ax_3d.plot_surface(data['X'], data['Y'], data['Z'], cmap='viridis')
        ax_3d.set_xlabel('x')
        ax_3d.set_ylabel('y')
        ...
        cp = ax_contour.contour(data['X'], data['Y'], data['Z'], 20, cmap='viridis')
        plt.colorbar(cp)
        ax_contour.set_xlabel('x')
        ax_contour.set_ylabel('y')
        
        # Plot gradient
        ax_contour.quiver(x0, y0, data['grad_f_x0_y0'][0], data['grad_f_x0_y0'][1], color='r', scale=10)
