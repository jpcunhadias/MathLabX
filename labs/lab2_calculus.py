...
def run_lab2():
    ...
    sub_lab = st.radio(
        "Select a page:",
        ("Function & Derivative", "Limits", "Integrals & Area"),
    )
    ...
    if sub_lab == "Function & Derivative":
        ...
        h_left = st.slider(
            "h_left (approaching 0 from the left)",
            min_value=0.0,
            max_value=1.0,
            value=0.1,
            step=0.01,
        ...
        h_right = st.slider(
            "h_right (approaching 0 from the right)",
            min_value=0.0,
            max_value=1.0,
            value=0.1,
            step=0.01,
        ...
        try:
            ...
            st.latex(
                f"\\lim_{{x \\to {a}^-}} {latex(f)} = {latex(left_limit)}"
            )
            st.latex(
                f"\\lim_{{x \\to {a}^+}} {latex(f)} = {latex(right_limit)}"
            )
            ...
            ax.scatter(
                [a - h_left, a + h_right], [f_a_left, f_a_right], color="red"
            )
            ax.set_xlabel("x")
            ax.set_ylabel("f(x)")
            ax.legend()
        ...
        n = st.slider(
            "Number of rectangles (n)", min_value=1, max_value=100, value=10
        )
        method = st.selectbox(
            "Riemann sum method", ("left", "midpoint", "right")
        )
        ...
        try:
            ...
            st.latex(
                f"\\int {latex(f)}\\,dx = {latex(indefinite_integral)}"
            )
            st.latex(
                f"\\int_{{{a}}}^{{{b}}} {latex(f)}\\,dx = "
                f"{latex(definite_integral)}"
            )
                    ...
                    Rectangle(
                        (
                            ...
                            - (
                                dx
                                if method == "right"
                                else 0 if method == "left" else dx / 2
                            )
                        ),
                        0,
                        dx,
